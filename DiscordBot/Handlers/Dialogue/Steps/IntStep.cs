        ...
        public override async Task<eDialogueType> ProcessStep(DiscordClient client, DiscordChannel channel, DiscordUser user)
        {
            ...
            while (true)
            {
                ...
                if (_minValue.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input is {inputValue} is smaller than {_minValue}").ConfigureAwait(false);
                        continue;
                    }
                }
                if (_maxValue.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input is {inputValue} is larger than {_maxValue}").ConfigureAwait(false);
                        continue;
                    }
                }
